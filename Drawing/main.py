...
def fill(spot, grid, color, c):
    if spot.color != c:
        ...
        i = (
            spot.col
        )  # the var i is responsible for denoting the current col value in the grid
        j = (
            spot.row
        )  # the var j is responsible for denoting the current row value in the grid
...
def save(cols, rows, show, grid, path):
    if (
        len(path) >= 4
    ):  # This just makes sure we have .txt at the end of our file selection
        if path[-4:] != ".txt":
            path = path + ".txt"
    else:
        path = path + ".txt"
...
def openFile(path):
    ...
    if f[-1] == str(currentVersion):
        ...
        for i in range(
            columns
        ):  # For every pixel, read the color and format it into a tuple
            for j in range(rows):
                line += 1
                nColor = []
...
def showFileNav(op=False):
    ...
    if op:
        filename = askopenfilename(
            title="Open File", filetypes=myFormats
        )  # Ask the user which file they want to open
    else:
        filename = asksaveasfilename(
            title="Save File", filetypes=myFormats
...
def updateLabel(a, b, c):
    ...
    label1.config(
        text="Pixel Size: " + str(l) + ", " + str(w)
    )  # Change label to show pixel size
        ...
        if event.type == pygame.QUIT:
            ...
            if pygame.display.get_caption()[0].count("*") > 0:
                if messagebox.askyesno(
                    "Save Work?", "Would you like to save before closing?"
                ):
        ...
        if pygame.mouse.get_pressed()[
            ...
            try:
                ...
                if pos[1] >= grid.height:  # If the mouse is below the main drawing grid
                    if (
                        pos[0] >= tools.startx
                        and pos[0] <= tools.startx + tools.width
                        and pos[1] >= tools.starty
                        and pos[1] < +tools.starty + tools.height
                    ...
                    # If they click on the color pallet
                        ...
                        pos[0] >= pallet.startx
                        and pos[0] <= pallet.startx + pallet.width
                        and pos[1] >= pallet.starty
                        and pos[1] <= pallet.starty + pallet.height
                    ):
                        ...
                        pos[0] >= lineThickness.startx
                        and pos[0] <= lineThickness.startx + lineThickness.width
                        and pos[1] >= lineThickness.starty
                        and pos[1] <= lineThickness.starty + lineThickness.height
                    ):
                    ...
                    # If they click on the save menu
                        ...
                        pos[0] >= saveMenu.startx
                        and pos[0] <= saveMenu.startx + saveMenu.width
                        and pos[1] >= saveMenu.starty
                        and pos[1] <= saveMenu.starty + saveMenu.height
                    ):
                        ...
                        if clicked.text == "Save":  # save if they click save
                            ...
                            if path:
                                savedPath = path
                                save(
                                    cols, rows, grid.showGrid, grid.getGrid(), savedPath
                                )
                        else:  # otherwise open
                    ...
                    if (
                        replace
                    ):  # If we have the replace tool selected then replace the color
                        tools.drawGrid()  # Redraw the grid so that we dont see the red highlight
                        buttons = ["D", "E", "F", "R", "C"]
                        tools.setText(buttons)
