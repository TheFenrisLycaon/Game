        ...
        for i in range(self.cols):
            ...
            for j in range(self.rows):
                ...
                pygame.draw.rect(
                    self.screen,
                    (0, 0, 0),
                    (x, y, self.width / self.cols, self.height / self.rows),
                    1,
                )
    ...
    def clearGrid(
        ...
        for pixels in self.grid:
            for p in pixels:
                if (
                    self.showGrid
                ):  # If the grid is to be showing we must redraw the pixels so that we can see the grid after we change their color
...
class textObject:
    ...
    def show(
        self, screen, color, st, outline=False
...
class pixel:
    def __init__(
        self, i, j, width, height, cols, rows, startx=0, starty=0, showGrid=False
    ):
    ...
    def show(
        self, screen, color, st, outline=False, first=False
    ...
    def click(
        self, screen, color
    ):  # If the pixel has been clicked on call this and it will display the new color and set the color attribute for that pixel
